      ...
      <ul className="divide-y divide-border mt-3">
        {items.map((item) => (
          <li key={`${item.type}-${item.id}`}>
            <Link href={item.cta_href} className="flex items-center justify-between gap-3 px-5 py-4 hover:bg-surface">
              ...
              <span className="shrink-0 text-xs font-medium text-brand-dark whitespace-nowrap">{item.cta_label} →</span>
            </Link>
          </li>
        ))}
      </ul>
